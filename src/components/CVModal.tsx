    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-[#4A4E69]/70 backdrop-blur-xs overflow-y-auto">
        <motion.div
          ...
          className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[92vh] flex flex-col relative my-auto border border-[#E9E9ED] overflow-hidden"
        >
          {/* Modal Header */}
          <div className="p-4 sm:p-5 bg-[#4A4E69] text-white flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-[11px] text-[#D4D4DB] font-mono">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrint}
                className="px-3 py-1.5 rounded-full bg-white/10 hover:bg-[#D4D4DB] hover:text-[#4A4E69] text-white text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 transition-colors cursor-pointer"
              >
              ...
              <button
                onClick={onClose}
                className="p-1.5 rounded-full bg-white/10 hover:bg-[#EFEFF2] hover:text-[#4A4E69] text-white transition-colors cursor-pointer"
          ...
          <div className="p-6 sm:p-10 overflow-y-auto font-sans text-[#4A4E69] text-xs sm:text-sm leading-relaxed space-y-6 print:p-0 print:overflow-visible">
            ...
            {/* Header / Name Section */}
            <div className="border-b border-[#E9E9ED] pb-6 flex flex-col sm:flex-row justify-between items-start gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-extrabold text-[#4A4E69] font-serif italic">
                  {PROFILE_DATA.name}
                </h1>
                <p className="text-xs font-bold text-[#8B8B95] tracking-widest uppercase mt-0.5">
                  {PROFILE_DATA.role}
                </p>
            ...
            {/* Summary */}
            <div>
              <h2 className="text-xs font-bold uppercase tracking-widest text-[#4A4E69] border-b-2 border-[#D4D4DB] pb-1 mb-2 font-mono">
                SUMMARY
              </h2>
            ...
            <div>
              <h2 className="text-xs font-bold uppercase tracking-widest text-[#4A4E69] border-b-2 border-[#D4D4DB] pb-1 mb-3 font-mono">
                PROJECT EXPERIENCE
              </h2>
              <div className="space-y-4">
                {PROJECTS.map((proj) => (
                  <div key={proj.id} className="p-3.5 rounded-2xl bg-[#FAFAFA] border border-[#E9E9ED]">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between font-bold text-[#4A4E69]">
                      <span className="font-serif italic">{proj.title} — <span className="text-[#8B8B95] font-normal">{proj.role}</span></span>
                      <span className="text-xs text-[#8B8B95] font-mono font-normal">{proj.period}</span>
                    </div>
                    <div className="text-xs text-[#8B8B95] font-medium mb-1.5">{proj.subtitle}</div>
            ...
            <div>
              ...
              <div className="flex justify-between items-center font-bold text-[#4A4E69]">
                <div>
                  ...
                  <div className="text-xs text-[#8B8B95] font-semibold">IPK: {PROFILE_DATA.gpa}</div>
                </div>
                <div className="text-xs text-[#8B8B95] font-mono font-normal">2023 - Present</div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {CERTIFICATES.map((cert) => (
                  <div key={cert.id} className="p-2.5 rounded-xl border border-[#E9E9ED] bg-[#FAFAFA] text-xs">
                    <strong className="block text-[#4A4E69] font-serif italic">{cert.title}</strong>
                    <span className="text-[#8B8B95] font-medium">{cert.issuer}</span> ({cert.issueDate})
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Footer Close */}
          <div className="p-4 bg-[#FAFAFA] border-t border-[#E9E9ED] flex justify-end">
            <button
              onClick={onClose}
