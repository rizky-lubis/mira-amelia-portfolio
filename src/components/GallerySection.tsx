...
export const GallerySection: React.FC = () => {
  ...
  return (
    <section id="gallery" className="py-20 relative bg-[#FAFAFA]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-[#4A4E69] font-display">
            Galeri Foto <span className="font-serif italic text-[#8B8B95] font-normal border-b-2 border-[#EFEFF2]">Karya Artistik</span>
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {galleryList.map((item, idx) => (
            <motion.div
              ...
              className="group relative rounded-3xl overflow-hidden cursor-pointer border border-[#E9E9ED] bg-white shadow-sm hover:border-[#D4D4DB] transition-all duration-300"
            >
              {/* Image Container */}
              <div className="aspect-[4/3] overflow-hidden bg-[#FAFAFA] relative">
                ...
                <div className="absolute top-3 right-3 w-8 h-8 rounded-full bg-white/90 flex items-center justify-center text-[#4A4E69] opacity-0 group-hover:opacity-100 transition-opacity transform group-hover:scale-105">
                  <Maximize2 className="w-4 h-4 text-[#8B8B95]" />
                </div>

                {/* Title & Description at bottom */}
                <div className="absolute bottom-4 left-4 right-4 text-white">
                  <h3 className="text-base font-bold font-serif italic group-hover:text-[#D4D4DB] transition-colors">
                    {item.title}
                  </h3>
        ...
        {selectedImage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#4A4E69]/80 backdrop-blur-md">
            <motion.div
              ...
              className="relative max-w-4xl w-full bg-white rounded-3xl overflow-hidden border border-[#E9E9ED] shadow-2xl"
            >
              <button
                onClick={() => setSelectedImage(null)}
                className="absolute top-4 right-4 z-10 p-2 rounded-full bg-white/90 hover:bg-[#EFEFF2] text-[#4A4E69] transition-colors cursor-pointer"
              ...
              <div className="grid grid-cols-1 md:grid-cols-12">
                ...
                <div className="md:col-span-4 p-6 flex flex-col justify-between bg-white">
                  <div>
                    <span className="px-3 py-1 rounded-full bg-[#E9E9ED] text-[#4A4E69] text-xs font-bold uppercase tracking-wider inline-block mb-3">
                  ...
                  <div className="pt-4 border-t border-[#E9E9ED] flex items-center justify-between">
                    <span className="text-[10px] text-[#8B8B95] font-mono uppercase tracking-wider">Mira Amelia Portfolio</span>
                    <button
                      onClick={() => setSelectedImage(null)}
                      className="px-4 py-2 rounded-2xl bg-[#8B8B95] hover:bg-[#4A4E69] text-white text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer"
                    >
                      Tutup
                    </button>
                  </div>
